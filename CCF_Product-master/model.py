...
def run(feature_files, training_dates, feature_set_folder):
    train_set = pd.concat(
        [dfs(0, len(feature_files),
         feature_files + ['y'], 'dataset/' + date) for date in training_dates])
   # print(train_set)
    test_set = dfs(0, len(feature_files), feature_files + ['y'], 'dataset/2020-04-02')
    test1_set = dfs(0, len(feature_files), feature_files + ['y'], 'dataset/2020-04-02')
    ...
    #print('feature_set :',feature_list)

    #train_set[feature_list].to_csv('dataset/train_set.csv')

    # end
    feature_data =  train_set[list(feature_set)]


    scaler = StandardScaler()
    scaler.fit(feature_data.as_matrix())

    # model1
    model1 = LinearRegression(normalize=True)
    set_x = scaler.transform(feature_data.as_matrix())

    print('set_x:',set_x.shape,'train_set[\'y\']:',train_set['y'].shape)

    weights = list(map(lambda x: 1.0 / x / x, train_set['y']))
    model1.fit(set_x, train_set['y'].as_matrix(),
...
def submit(feature_files, training_dates, feature_set_folder):
    ...
    # model1
    model1 = LinearRegression(normalize=True)
    set_x = scaler.transform(train_set[feature_set].as_matrix())
    model1.fit(set_x, train_set['y'].as_matrix(),
               sample_weight=map(lambda x: 1.0 / x / x, train_set['y'].as_matrix())
               )
    print (zip(feature_set, model1.coef_))
...
'''
run(['v1', 'v3', 'v11'], ['2020-03-01', '2020-02-01'], 'feature_set1')
submit(['v1', 'v3', 'v11'], ['2020-04-01', '2020-02-01'], 'feature_set1')
